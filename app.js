...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchData() {
    try {
      const [healthRes, riskRes, metricsRes, incidentsRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/api/risk/current`),
        fetch(`${API_BASE}/api/metrics/recent`),
      ...
      statusText.textContent = `System ${health.status || 'Active'}`;
      ...
      document.getElementById('system-status-text').textContent = 'Sync offline';
  ...
  function updateRiskUI(data) {
    ...
    if (data.latest_incident) {
      ...
      if (data.severity === 'HIGH' || data.severity === 'CRITICAL') {
        cardEl.classList.add('border-l-rose-500', 'bg-rose-500/5');
        cardEl.classList.remove('border-l-emerald-500', 'border-l-amber-500');
        iconEl.className = 'fas fa-triangle-exclamation text-rose-500';
      } else {
        cardEl.classList.add('border-l-amber-500', 'bg-amber-500/5');
        cardEl.classList.remove('border-l-emerald-500', 'border-l-rose-500');
  ...
  function updateMetricsUI(data) {
    ...
    errorHistory = reversed.map(m => m.error_rate * 10);
  ...
  function updateIncidentsUI(data) {
    ...
    if (!data || data.length === 0) {
      table.innerHTML = '<tr><td colspan="6" class="px-6 py-10 text-center text-slate-500 italic">No historical data</td></tr>';
      return;
    }
    ...
    table.innerHTML = data.map(inc => `
            <tr class="hover:bg-white/5 transition-colors">
                ...
                <td class="px-6 py-4 text-right">
                    <button class="text-slate-500 hover:text-sky-400 transition-colors"><i class="fas fa-arrow-right"></i></button>
                </td>
            </tr>
        `).join('');
  }
